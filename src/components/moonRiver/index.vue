  ...
  methods: {
        ...
        moonriverService.getRealtimeCollatorCandidatePool().then((c) => {
          ...
          Promise.all(allPromiseArr).then((d) => {
            ...
            nominatorRes.forEach((v) => {
              ...
              v.topNominators.forEach((sv) => {
                sv.amount = BigNumber(sv.amount, 16).dividedBy(1e18);
              });
              v.bottomNominators.forEach((sv) => {
                sv.amount = BigNumber(sv.amount, 16).dividedBy(1e18);
              });
              v.allNominators = [...v.topNominators, ...v.bottomNominators];
              //排序小弟
            ...
            nominatorRes.forEach((v) => {
              ...
              for (
                ...
                const find = getCollector10RewardRes.rewards.find(
                  (sv) => sv.account.toLowerCase() == v.id.toLowerCase() && sv.roundIndex == i
                );
                if (find) {
                  arr.push({
                    roundIndex: i,
    ...
    generateTableChart() {
      this.tableData.forEach((v) => {
        const tableChart = echarts.init(this.$refs[`tableChart${v.id}`]);
