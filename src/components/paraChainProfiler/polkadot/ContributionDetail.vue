<template>
  <div class="content contributionDetail">
      ...
      <div class="header-filter">
        ...
        <div class="filter-right">
          ...
          <div class="contribution-totalAmount">
            Total: {{ contributionAmount | formatDOT }} &nbsp;DOT
          </div>
          <div
            class="contribution-contributorStatisticDisplay"
            v-html="getContributorStatisticDisplayText()"
          ></div>
          <div class="reset">
    ...
    <div class="g-wrap">
      ...
      <div class="chart">
        <h2 class="title" style="position: relative">
          Top 5 contributors by DOT contributed
          <div class="share-wrap">
            <img
              title="Share to Twitter"
              @click="
...
export default {
  ...
  methods: {
    ...
    formatedTokenValue(v) {
      return utils.formatToken(v, utils.DOT_RATIO);
    },
    refreshAllData() {
      let self = this;
      Promise.all([
    ...
    formatedCap(row, isEchartData) {
      let result = utils.formatedDisplay4CrowdloanDOT(row, isEchartData);
    ...
    initHistory(data) {
      ...
      const historyTotalOption = {
        title: {
          text: "Total DOT raised during the crowdloan",
          left: -5,
          textStyle: {
            color: "#292828",
          },
      ...
      const historyIncrementOption = {
        title: {
          text: "Daily increase of DOT raised during the crowdloan",
          left: -5,
          textStyle: {
            color: "#292828",
          },
    ...
    async refreshData(resetPageIndex) {
      ...
      try {
        ...
        const obj = res.find((v) => v.network == "polkadot");
        searchKey = obj.value;
      } catch (err) {
    ...
    update(tableData) {
      ...
      for (let index = 0; index < tableData.length; index++) {
        ...
        row.amountFormated = new BigNumber(row.amount).div(
          new BigNumber(this.tokenRatio)
        );

        let precision = 2;
        row.amountFormated = row.amountFormated.toFormat(precision) + " DOT";
      }
    },
  },
};
...
.header-filter .filter-right .contribution-contributorStatisticDisplay {
  margin: 10px 10px;
  color: #000;
  i {
    font-size: 16px;
